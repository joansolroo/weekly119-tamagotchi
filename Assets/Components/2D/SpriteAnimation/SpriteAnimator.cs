...
public class SpriteAnimator : MonoBehaviour
{
    ...
    void Tick()
    {
        ...
        if (firstFrame || frame != currentFrame)
        {
            ...
            if(currentAnimation.audio.Length> currentFrame && currentAnimation.audio[currentFrame] != null)
            {
                if (audioSource != null)
                {
                    SoundClip clip = currentAnimation.audio[currentFrame];
                    audioSource.pitch = Random.Range(clip.pitchMin, clip.pitchMax);
                    audioSource.volume = clip.volume;
                    audioSource.PlayOneShot(clip.clip);
                    Debug.Log("" + currentAnimation.name + ".soundClip(" + currentFrame + ")");
                }
            }
        }
    }
